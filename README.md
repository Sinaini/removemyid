...
3. **Redact:**
   - `.txt` / `.csv` — text is sent to a **Web Worker** running a regex pipeline (email/phone/SSN/credit card/date/age) plus [compromise.js](https://github.com/spencermountain/compromise) for name/place NER. Matches are replaced with `[REDACTED]`.
   - `.pdf` — [pdfjs-dist](https://github.com/mozilla/pdf.js) extracts per-page text with character offsets, the same detection pipeline finds PII in it, each page is rendered to a canvas, black boxes are drawn over the matched spans using real font-metric measurement (not just character-count guessing), and the flattened page image is embedded into a brand-new PDF via [pdf-lib](https://github.com/Hopding/pdf-lib). The original text layer is discarded entirely.
